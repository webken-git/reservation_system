...
from rest_framework import viewsets, response, status, mixins, views
...
def create_new_word(request):
  ...
  return {'path': tbl.rows[3].cells[1].paragraphs[0].text}
...
class CreateNewDocumentView(views.APIView):
    # permission_classes = [IsAuthenticated]
  serializer_class = DocumentSerializer
