...
const ApprovalInfo = (props) => {
  ...
  const pullReservation = () => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_API}/api/reservations/${id}`, {})
      .then((res) => {
  ...
  const getUsage = (id) => {
    axios
      .get(
        `${process.env.REACT_APP_API}/api/usage-categories/?reservation=${id}`
      )
      .then((res) => {
        setUsage(res.data);
        // console.log(res.data);
  ...
  const getAge = (id) => {
    axios
      .get(`${process.env.REACT_APP_API}/api/age-categories/?reservation=${id}`)
      .then((res) => {
        setAge(res.data);
      })
      .catch((err) => {});
          ...
          <table className="info-table">
            <tbody>
              ...
              <tr>
                <td>予約日：</td>
                <td>
                  {formatDate(new Date(reservation.start.replace(/-/g, "/")))}
                </td>
              </tr>
              <tr>
                <td>予約時間：</td>
