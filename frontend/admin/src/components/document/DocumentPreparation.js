...
const DocumentPreparation = (props) => {
    ...
    const createDocument = () => {
        props.document.id.map(async (id) => {
            const response = await axios.post(post_DocumentUrl,
                { id: id, number: props.document.number, approval_application_id: 2 })
                .then(res => {
                    console.log(res);
    ...
    return (
        <div className="document-preparation">
            {/* {error && <div className="document-preparation__error">{error.error}</div>} */}
            <div className="document-preparation__header">
                <h3 className="document-preparation__title">
                    {error && <span>{error.error}</span> || error === null && <span>申請書の発行が完了しました。</span>}
                </h3>
            </div>
