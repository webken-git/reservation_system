...
export const DataList = () => {
  ...
  const [divideEquipmentFeeList, setDivideEquipmentFeeList] = useSafeState(unmountRef, []);
  ...
  const GetEquipmentFeeList = () => {
    axios
      .get(ReservationUrls.EQUIPMENT_FEE)
      .then((response) => {
        const equipmentFeelists = response.data;
        setEquipmentFeeList(equipmentFeelists);
        // console.log(equipmentFeelists)
      })
      .catch((error) => { });
  }
  ...
  const fees = placeListData.map((place, p_id) => {
    ...
    const FacilityFee = () => {
      if (isGroup.length === 0) {
        return <FeeList key={p_id} feelist={feeData[0].data} placeid={place.id} age={age} />;
      } else if (place.max > 1) {
        return <CurlingFeeList key={p_id} feelist={feeData[0].data} placeid={place.id} age={age} />
      } else {
        return <GroupFeeList key={p_id} feelist={feeData[0].data} placeid={place.id} age={age} />;
      }
    };
